...
## Mascot placement revision
All information boards now anchor the `devil-around-board.png` asset to the top edge so the mascot reads as coming around / over the board. Fresh Souls mirrors the same asset from the opposite side. Daily Match Champs keeps the separate pointing-at-winners asset, which was already correctly placed.


## Visual update
- Mascot appears only twice: Fresh Souls and Daily Match Champs.
- Board subheadings removed.
- Infernal Ascent ranks members by actual Daily Elo improvement between stored snapshots.
- Daily Match Champs shows player results only; explanatory placeholder copy was removed.
